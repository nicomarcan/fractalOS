...
static int initValidMoves (Game * g)
{
	Moves * v_moves;
    int err_flag;
	g->v_moves = calloc(1, sizeof( *g->v_moves));
	if (g->v_moves == NULL)
	{
		return;
	}
...
{
	g->turn_path = calloc(1,sizeof(*g->turn_path));
	if (g->turn_path == NULL)
	{
		return;
	}
...
static int addToTurnPath(TurnPath * turn_path, Chip * chip)
{
     ...
     {
        count = n + MEM_BLOCK;
        aux = realloc(turn_path->chipset, sizeof(*(turn_path->chipset))*count);
        if ( aux == NULL)
        {
           return MEM_ERROR;
        }
        turn_path->chipset = aux;
     }
...
static int validMoves (Game * g)
{
     ...
     if (flag != MEM_ERROR){
         ...
         moveset = realloc(moveset, sizeof(*moveset)*g->v_moves->count);
     }
     return flag;
}
    ...
    paikaMove(Game * g, Move m, int dir[2],int careful, int * careful_moves)
{
    ...
    if ( softCapture(g,m,dir) == FALSE)
    {
        ...
        if (( count % BLOCK)==0)
        {
            aux = realloc(moveset, sizeof(*moveset) * (count + BLOCK));
            if (aux==NULL)
            {
               return MEM_ERROR;
            }
            g->v_moves->moveset = aux;
        }
        move = assignMove(m,0) ;
        if ( move == NULL)
        {
